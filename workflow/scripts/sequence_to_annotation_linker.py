...
parser.add_argument('imap', help='mapping file for the internal clusters as made by the get_taxonomy rule')
...
imap=pl.read_csv(args.imap, separator=',', has_header=True)

functions.columns=["query","linker_info"]
imap=imap["cluster","target"].unique()
imap.columns=["cluster","node_head"]

merged=functions.join(linker, on="linker_info")
merged.columns=["linker_info","node_head","gene_id","accession","function"]

merged=merged.join(imap, on="node_head")
merged=merged["cluster","node_head","accession","function"].unique()
merged.columns=["linker_info","node_head","accession","function"]

merged.write_csv(args.output, include_header=True)
